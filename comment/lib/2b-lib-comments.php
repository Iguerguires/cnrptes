...
class Comments {
  ...
  function get($pid = 0) {
    ...
    $sql = "SELECT * FROM `comments` WHERE `post_id`=? and id_theme = 1 ORDER BY `reponse_id`";
    $this->stmt = $this->pdo->prepare($sql);
    $this->stmt->execute([$pid]);
    $comments = [];
    while ($row = $this->stmt->fetch(PDO::FETCH_NAMED)) {
      if (is_numeric($row['reponse_id'])) {
        $comments[$row['reponse_id']]['reply'][$row['id_comment']] = $row;
      } else {
  ...
  //       $message : comment message
    ...
    $sql = "UPDATE `comments` SET `name`=?, `message`=? WHERE `id_comment`=?;";
  ...
  // PARAM $cid : comment ID
    ...
    $this->start();
    $pass = $this->exec("DELETE FROM `comments` WHERE `id_comment`=?;", [$cid]);
    if ($pass) {
      $pass = $this->exec("DELETE FROM `comments` WHERE `reponse_id`=?;", [$cid]);
    }
    $this->end($pass);
    return $pass;
  }
